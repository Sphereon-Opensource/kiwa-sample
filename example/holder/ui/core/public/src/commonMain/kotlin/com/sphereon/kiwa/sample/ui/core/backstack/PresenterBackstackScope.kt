...
import com.sphereon.kiwa.sample.ui.core.backstack.PresenterBackstackScope.BackstackChange
import com.sphereon.kiwa.sample.ui.core.backstack.PresenterBackstackScope.BackstackChange.Action
import com.sphereon.kiwa.sample.ui.core.saveable.rememberReturningSaveableStateHolder
...
private class PresenterBackstackScopeImpl(val initial: MoleculePresenter<*, out BaseModel>) :
    ...
    private var _lastBackstackChange =
        mutableStateOf(
            BackstackChangeImpl(
                listOf(initial) as List<MoleculePresenter<Any, out BaseModel>>,
                BackstackChange.Action.PUSH
            )
        )
    ...
    override fun push(presenter: MoleculePresenter<*, out BaseModel>) {
        _lastBackstackChange.value =
            BackstackChangeImpl(
                ...
                action = BackstackChange.Action.PUSH,
            )
    }
    ...
    override fun pop() {
        ...
        if (oldStack.size > 1) {
            _lastBackstackChange.value =
                BackstackChangeImpl(backstack = oldStack.subList(0, oldStack.size - 1), action = BackstackChange.Action.POP)
        }
    }

    override fun clear() {
        _lastBackstackChange.value =
            BackstackChangeImpl(listOf(initial) as List<MoleculePresenter<Any, out BaseModel>>, BackstackChange.Action.PUSH)
    }

    private class BackstackChangeImpl(
        override val backstack: List<MoleculePresenter<Any, out BaseModel>>,
        override val action: BackstackChange.Action,
    ) : BackstackChange
}
